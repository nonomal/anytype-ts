...
const PopupSettingsSpaceShareTG = observer(class PopupSettingsSpaceShareTG extends React.Component<I.PopupSettings, State> {
    ...
    render () {
        const { isLoading, error, cid, key, nftTokenAddr } = this.state;
    ...
    init () {
        const { cid, key, nftTokenAddr } = this.state;
        const space = U.Space.getSpaceview();

        if (space.isShared && !cid && !key) {
            C.SpaceInviteGetCurrent(S.Common.space, (message: any) => {
                if (!message.error.code) {
                    this.setInvite(message.inviteCid, message.inviteKey, nftTokenAddr);
                };
            });
        };
    };

    setInvite (cid: string, key: string, nftTokenAddr: string) {
        this.setState({ cid, key, nftTokenAddr });
    };
    ...
    onCopy () {
        ...
        U.Common.copyToast('', U.Space.getInviteLink(cid, key, nftTokenAddr), translate('toastInviteCopy'));
    ...
    onInitLinkTG () {
        ...
        const tokenAddress = this.refInput2?.getValue()?.toString();
        ...
        this.refButton2?.setLoading(true);

        C.SpaceMakeShareable(S.Common.space, (message: any) => {
            if (this.setError(message.error)) {
                this.refButton2?.setLoading(false);
                return;
            };
            ...
            C.SpaceInviteGenerate(S.Common.space, (message: any) => {
                ...
                if (!this.setError(message.error)) {
                    this.setInvite(message.inviteCid, message.inviteKey, tokenAddress);
...
export default PopupSettingsSpaceShareTG;
